...
export class SuggestionEngine {
  ...
  async runLearningCycle(): Promise<{
    ...
    if (this.config.enabledModules.includes('threat')) {
      ...
      const llmEnrich = process.env.MASTYFF_AI_AI_THREAT_LLM !== 'false';
...
export async function initializeAiEngine(
  ...
  const useDbSnapshots =
    process.env.MASTYFF_AI_AI_USE_DB_SNAPSHOTS !== 'false'
    || process.env.MASTYFF_AI_AI_SKIP_EXTERNAL_FETCH === 'true';
  ...
  const engineConfig: Partial<SuggestionEngineConfig> = {
    analysisIntervalMs: parseInt(process.env.MASTYFF_AI_AI_ANALYSIS_INTERVAL_MS || String(15 * 60 * 1000), 10),
    autoApplyThreshold: parseFloat(process.env.MASTYFF_AI_AI_AUTO_APPLY_THRESHOLD || '0.85'),
    policyOutputPath: process.env.MASTYFF_AI_AI_POLICY_OUTPUT || './default-policy-auto-generated.yaml',
  };
  ...
  if (process.env.MASTYFF_AI_AI_DISABLE_PERIODIC !== 'true') {
  ...
  if (process.env.MASTYFF_AI_AI_SKIP_INITIAL_CYCLE !== 'true') {

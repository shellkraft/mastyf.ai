...
export class ComprehensiveReporter {
  ...
  toPlainText(report: ComprehensiveReport): string {
    const lines: string[] = [];
    lines.push('MCP Guardian — Comprehensive Analysis');
    ...
    if (report.executiveSummary.topRisks.length > 0) {
      lines.push('');
      lines.push('TOP RISKS');
      for (const r of report.executiveSummary.topRisks) lines.push(`  - ${r}`);
    }
    if (report.executiveSummary.recommendations.length > 0) {
      lines.push('');
      lines.push('SUMMARY RECOMMENDATIONS');
      for (const r of report.executiveSummary.recommendations) lines.push(`  - ${r}`);
    }
    lines.push('');
    lines.push('SECURITY');
    for (const s of report.security.serverReports) {
      lines.push(
        `  ${s.name}: score ${s.score}/100, ${s.cves} CVE(s) (${s.critical} critical), auth ${s.auth ? 'yes' : 'no'}`,
      );
    }
    ...
    lines.push(`  Projected monthly: $${report.cost.projectedMonthly.toFixed(2)}`);
    for (const a of report.cost.budgetAlerts) lines.push(`  Alert: ${a}`);
    lines.push('');
    lines.push('HEALTH');
    for (const h of report.health.serverReports) {
    ...
    if (report.behavioral.baselines.length > 0) {
      ...
      for (const b of report.behavioral.baselines.slice(0, 12)) {
        lines.push(
          `  ${b.server}/${b.tool}: ${b.sampleCount} samples, ~${b.avgTokens} tokens, ~${b.avgLatency}ms`,
        );
      }
    ...
    if (report.policy.autoGeneratedRules.length > 0) {
      ...
      for (const rule of report.policy.autoGeneratedRules.slice(0, 10)) lines.push(`  - ${rule}`);
    }
    return lines.join('\n');
  }
}
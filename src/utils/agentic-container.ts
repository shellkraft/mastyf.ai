...
export async function ensureAgenticContainer(): Promise<Container | null> {
  ...
  if (process.env.GUARDIAN_AGENTIC_ENABLED === 'false') return null;
  if (!_initPromise) {
    _initPromise = (async () => {
      try {
        const { createContainer } = await import('../container.js');
        const dbPath = process.env.MCP_GUARDIAN_DB_PATH;
...
/** Default: enabled when container is set. Set GUARDIAN_AGENTIC_ENABLED=false to disable hooks. */
export function isAgenticEnabled(): boolean {
  if (process.env.GUARDIAN_AGENTIC_ENABLED === 'false') return false;
  return _agenticContainer != null;
}

export function isAgenticDemoMode(): boolean {
  return process.env.GUARDIAN_AGENTIC_DEMO_MODE === 'true';
}

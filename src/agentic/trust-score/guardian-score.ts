/**
 * MCP Guardian Trust Score — Like SSL Labs for MCP servers.
 *
...
export interface ScoreInput {
  ...
  /** Whether the server is behind a Guardian proxy */
  guardianProtected: boolean;
}

export class GuardianScore {
  /**
  ...
  /** Score Guardian protection layer (0-100, weight 0.05) */
  private scoreProtectionLayer(input: ScoreInput): ScoreCategory {
    return {
      name: 'Guardian Protection',
      score: input.guardianProtected ? 100 : 0,
      weight: 0.05,
      maxScore: 100,
      details: `Behind Guardian: ${input.guardianProtected ? 'Yes' : 'No'}`,
      findings: [input.guardianProtected ? 'Protected by MCP Guardian proxy' : 'Not behind Guardian — no policy enforcement, no audit, no DLP'],
    };
  }
      ...
      if (cat.score < cat.maxScore * 0.5) {
        ...
        switch (cat.name) {
          ...
          case 'Transport Security':
            action = 'Upgrade to mTLS transport or route through Guardian proxy';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          ...
          case 'Response Hygiene':
            action = 'Enable Guardian Response DLP to prevent data leaks';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          case 'Guardian Protection':
            action = 'Place this server behind MCP Guardian proxy for policy enforcement, audit, and DLP';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          default:

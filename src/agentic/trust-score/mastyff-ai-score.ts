/**
 * MCP Mastyff AI Trust Score — Like SSL Labs for MCP servers.
 *
...
export interface ScoreInput {
  ...
  /** Whether the server is behind a Mastyff AI proxy */
  mastyffAiProtected: boolean;
}

export class MastyffAiScore {
  /**
  ...
  /** Score Mastyff AI protection layer (0-100, weight 0.05) */
  private scoreProtectionLayer(input: ScoreInput): ScoreCategory {
    return {
      name: 'Mastyff AI Protection',
      score: input.mastyffAiProtected ? 100 : 0,
      weight: 0.05,
      maxScore: 100,
      details: `Behind MastyffAi: ${input.mastyffAiProtected ? 'Yes' : 'No'}`,
      findings: [input.mastyffAiProtected ? 'Protected by MCP Mastyff AI proxy' : 'Not behind Mastyff AI — no policy enforcement, no audit, no DLP'],
    };
  }
  ...
  private computeBadge(grade: TrustScore['grade']): TrustBadge {
    ...
    return {
      ...
      text: `MCP Mastyff AI Score: ${grade}`,
    };
  }
      ...
      if (cat.score < cat.maxScore * 0.5) {
        ...
        switch (cat.name) {
          ...
          case 'Transport Security':
            action = 'Upgrade to mTLS transport or route through Mastyff AI proxy';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          ...
          case 'Response Hygiene':
            action = 'Enable Mastyff AI Response DLP to prevent data leaks';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          case 'Mastyff AI Protection':
            action = 'Place this server behind MCP Mastyff AI proxy for policy enforcement, audit, and DLP';
            expected = Math.round((100 - cat.score) * cat.weight);
            effort = 'hours';
            break;
          default:

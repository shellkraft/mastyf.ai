...
export type MastyffAiHeaders = Record<string, string>;

const TENANT_STORAGE_KEY = 'mastyff-ai-tenant-id';
...
export type MastyffAiFullAnalysisResponse = {
  available?: boolean;
  generatedAt?: string;
  windowDays?: number;
...
export function resolveApiBase(): string {
  ...
  const envBase = process.env.NEXT_PUBLIC_MASTYFF_AI_API;
...
export function buildAuthHeaders(): MastyffAiHeaders {
  const headers: MastyffAiHeaders = { Accept: 'application/json' };
  if (typeof window === 'undefined') return headers;
  const params = new URLSearchParams(window.location.search);
  const apiKey = params.get('apiKey');
  if (apiKey) headers['X-API-Key'] = apiKey;
  const tenant = getTenantId();
  headers['X-Mastyff-Ai-Tenant'] = tenant;
  headers['X-Tenant-Id'] = tenant;
  return headers;
}

export async function mastyffAiFetch(
  path: string,
  init?: RequestInit,
): Promise<Response> {
  ...
  return fetch(url, {
    ...
    headers: {
      ...buildAuthHeaders(),
      ...(init?.headers as MastyffAiHeaders),
    },
  });
}

export async function fetchAuthStatus(): Promise<AuthStatus> {
  const res = await mastyffAiFetch('/api/auth/status');
  if (!res.ok) {
...
export async function fetchCsrfToken(): Promise<{ csrfToken?: string; csrfEnforced: boolean }> {
  const res = await mastyffAiFetch('/api/auth/csrf');
...
export async function buildMutatingHeaders(
  extra: MastyffAiHeaders = {},
): Promise<MastyffAiHeaders> {
  const headers: MastyffAiHeaders = { 'Content-Type': 'application/json', ...extra };
...
export async function loginDashboard(body: {
  ...
  const headers: MastyffAiHeaders = { 'Content-Type': 'application/json' };
  if (body.csrfToken) headers['X-CSRF-Token'] = body.csrfToken;
  const res = await mastyffAiFetch('/api/login', {
    method: 'POST',
    headers,
    body: JSON.stringify({
...
export async function logoutDashboard(): Promise<void> {
  await mastyffAiFetch('/api/logout', { method: 'POST' });
}
...
export async function fetchTenantContext(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/admin/tenant');
...
export async function fetchDashboardRegions(): Promise<{ regions: string[] } | null> {
  const res = await mastyffAiFetch('/api/dashboard/regions');
  if (!res.ok) return null;
...
export async function fetchAggregateMetrics(windowDays = 7, region?: string): Promise<AggregateMetrics | null> {
  ...
  const res = await mastyffAiFetch(`/api/aggregate/metrics?${q}`);
  if (!res.ok) return null;
...
export async function fetchAudit(opts?: {
  ...
  const res = await mastyffAiFetch(`/api/aggregate/audit${q ? `?${q}` : ''}`);
  if (!res.ok) return null;
...
export async function fetchCost(windowDays = 7, region?: string): Promise<CostResponse | null> {
  const res = await mastyffAiFetch(`/api/cost?${withWindowRegionQuery(windowDays, region)}`);
  if (!res.ok) return null;
...
export async function fetchCostBreakdown(windowDays = 7): Promise<CostBreakdownResponse | null> {
  const res = await mastyffAiFetch(`/api/cost/breakdown?window=${windowDays}`);
  if (!res.ok) return null;
...
export async function fetchCostRecommendations(
  ...
  const res = await mastyffAiFetch(`/api/cost/recommendations?window=${windowDays}`);
  if (!res.ok) return null;
...
export async function fetchCostTimeseries(
  ...
  const res = await mastyffAiFetch(`/api/cost/timeseries?${q}`);
  if (!res.ok) return null;
...
export async function fetchExecutiveSummary(
  ...
  const res = await mastyffAiFetch(`/api/dashboard/executive-summary?${withWindowRegionQuery(windowDays, region)}`);
  if (!res.ok) return null;
...
export async function fetchDashboardInsights(
  ...
  const res = await mastyffAiFetch(`/api/dashboard/insights?scope=${scope}&window=${windowDays}`);
  if (!res.ok) return null;
...
export async function trackAdvancedAnalyticsEvent(event: {
  ...
  try {
    await mastyffAiFetch('/api/dashboard/analytics/telemetry', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ...event,
...
export async function downloadInsightsBriefing(
  ...
  const res = await mastyffAiFetch(`/api/dashboard/insights/export?scope=${scope}&window=${windowDays}`);
  if (!res.ok) return;
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `mastyff-ai-briefing-${scope}-${windowDays}d.md`;
  a.click();
  ...
  try {
    ...
    if (body?.error?.includes('Dashboard API disabled')) {
      return (
        ...
        'pnpm build:mastyff-ai && pnpm dashboard:proxy -- mastyff-ai-configs/filesystem.json ' +
        '(sets MASTYFF_AI_CI_BYPASS_LICENSE). Open http://localhost:4000/ — not a separate Next dev port unless you use ?apiBase=http://localhost:4000'
      );
    }
...
export async function fetchMcpHealthReport(
  ...
  const res = await mastyffAiFetch(
  ...
  if (res.status === 404) {
    return {
      ...
      error: await parseApiErrorMessage(
        res,
        'Health report API not found — run `pnpm build:mastyff-ai`, restart the proxy with DASHBOARD_ENABLED=true, then refresh.',
      ),
    };
  }
...
export async function downloadMcpHealthReport(windowDays = 7, useLlm = false): Promise<void> {
  ...
  a.download = `mastyff-ai-mcp-health-${date}.md`;
  a.click();
  URL.revokeObjectURL(url);
}
...
export async function fetchFullAnalysis(
  windowDays = 7,
  useLlm = true,
): Promise<{ analysis: MastyffAiFullAnalysisResponse | null; error?: string }> {
  const res = await mastyffAiFetch(
  ...
  if (res.status === 404) {
    return {
      ...
      error: await parseApiErrorMessage(
        res,
        'Full analysis API not found — run `pnpm build:mastyff-ai`, restart the proxy with DASHBOARD_ENABLED=true, then refresh.',
      ),
    };
  }
  let body: MastyffAiFullAnalysisResponse;
  try {
    body = (await res.json()) as MastyffAiFullAnalysisResponse;
  } catch {
...
export async function downloadFullAnalysis(windowDays = 7, useLlm = true): Promise<void> {
  ...
  a.href = url;
  const date = new Date().toISOString().slice(0, 10);
  a.download = `mastyff-ai-full-analysis-${date}.md`;
  a.click();
  URL.revokeObjectURL(url);
}
...
export async function fetchAuditHeatmap(
  ...
  const res = await mastyffAiFetch(`/api/audit/heatmap?${withWindowRegionQuery(window, region)}`);
  if (!res.ok) return null;
...
export async function fetchSecurity(): Promise<SecurityResponse | null> {
  const res = await mastyffAiFetch('/api/security');
  if (!res.ok) return null;
  return liveOrNull((await res.json()) as SecurityResponse);
}

export async function fetchHealth(): Promise<HealthResponse | null> {
  const res = await mastyffAiFetch('/api/health');
  if (!res.ok) return null;
...
export async function fetchFleetInstances(): Promise<FleetResponse | null> {
  const res = await mastyffAiFetch('/api/instances');
  if (!res.ok) return null;
...
export async function fetchAiSuggestions(): Promise<AiSuggestion[]> {
  const res = await mastyffAiFetch('/api/ai/suggestions');
  if (!res.ok) return [];
...
export async function testPolicy(payload: {
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/test', {
    method: 'POST',
    headers,
    body: JSON.stringify({
      tool: payload.tool,
...
export async function fetchPolicyCopilot(goal: string, availableTools?: string[]): Promise<Record<string, unknown> | null> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/copilot', {
    method: 'POST',
    headers,
...
export async function fetchPolicyCounterfactual(
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/copilot/counterfactual', {
    method: 'POST',
    headers,
...
export async function fetchComplianceReport(windowDays = 7): Promise<Record<string, unknown> | null> {
  const res = await mastyffAiFetch(`/api/ai/compliance/report?window=${windowDays}&useLlm=false`);
  if (!res.ok) return null;
  return (await res.json()) as Record<string, unknown>;
}

export async function fetchTenantModelReadiness(): Promise<TenantModelReadinessResponse | null> {
  const res = await mastyffAiFetch('/api/ai/tenant-model/readiness');
  if (!res.ok) return null;
...
export async function fetchTenantModelTrain(
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/tenant-model/train', {
    method: 'POST',
    headers,
...
export async function fetchTenantModelTrainStatus(): Promise<TenantModelTrainStatus | null> {
  const res = await mastyffAiFetch('/api/ai/tenant-model/train/status');
  if (!res.ok) return null;
...
export async function investigateIncident(triggerId: string): Promise<InvestigateIncidentResult> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/incidents/investigate', {
    method: 'POST',
    headers,
  ...
  if (!res.ok) {
    ...
    try {
      ...
      if (body.error === 'Not found') {
        message =
          'Incident API is unavailable on this dashboard host. Restart the Mastyff AI proxy after `pnpm build` so it loads the latest dashboard routes.';
...
export async function acceptSuggestion(suggestion: AiSuggestion): Promise<boolean> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/suggestions/accept', {
    method: 'POST',
    headers,
    body: JSON.stringify({
...
export async function rejectSuggestion(suggestion: AiSuggestion): Promise<boolean> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/suggestions/reject', {
    method: 'POST',
    headers,
    body: JSON.stringify({
      suggestionId: suggestion.id || suggestion.ruleName,
...
export async function reloadPolicy(): Promise<boolean> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/reload', { method: 'POST', headers });
  return res.ok;
}
...
export async function runSecuritySwarm(opts?: {
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/security-swarm/run', {
    method: 'POST',
    headers,
...
export async function fetchSwarmStatus(): Promise<SwarmJobStatus | null> {
  const res = await mastyffAiFetch('/api/security-swarm/status');
  if (!res.ok) return null;
...
export async function fetchSwarmJobLog(): Promise<SwarmJobLogResponse | null> {
  const res = await mastyffAiFetch('/api/security-swarm/job-log');
  if (!res.ok) return null;
...
export async function fetchSoarPlaybooks(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/soar/playbooks');
  if (!res.ok) return null;
...
export async function fetchSwarmReportPreview(): Promise<string | null> {
  const res = await mastyffAiFetch('/api/security-swarm/report');
  if (!res.ok) return null;
  return res.text();
}

export async function downloadSwarmReport(): Promise<void> {
  const res = await mastyffAiFetch('/api/security-swarm/report/download');
  if (!res.ok) return;
  const blob = await res.blob();
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'mastyff-ai-swarm-analysis.txt';
  a.click();
  URL.revokeObjectURL(url);
}

export async function fetchAiReport(): Promise<AiReport | null> {
  const res = await mastyffAiFetch('/api/ai/report');
  if (!res.ok) return null;
...
export async function fetchAiState(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/ai/state');
  if (!res.ok) return null;
...
export async function fetchAiBaselines(): Promise<unknown[]> {
  const res = await mastyffAiFetch('/api/ai/baselines');
  if (!res.ok) return [];
...
export async function fetchIntelQuarantinePolicy(
  ...
  const res = await mastyffAiFetch('/api/ai/threats/quarantine/policy', {
    method: 'POST',
    headers,
...
export async function fetchMonitorQuarantinePolicy(
  ...
  const headers = await buildMutatingHeaders({ 'Content-Type': 'application/json' });
  const res = await mastyffAiFetch('/api/security/threats/quarantine/policy', {
    method: 'POST',
    headers,
    body: JSON.stringify({
...
export async function fetchAiThreats(): Promise<ThreatIntelStatus | null> {
  const res = await mastyffAiFetch('/api/ai/threats');
  if (!res.ok) return null;
...
export async function pollAiThreats(): Promise<{ ok: boolean; status?: ThreatIntelStatus; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/threats/poll', { method: 'POST', headers, body: '{}' });
  if (!res.ok) return { ok: false, error: await parseApiError(res) };
...
export async function fetchQuarantinedThreats(days = 30): Promise<QuarantineRecord[]> {
  const res = await mastyffAiFetch(`/api/ai/threats/quarantined?days=${days}`);
  if (!res.ok) return [];
...
export async function quarantineThreatIntel(
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/threats/quarantine', {
    method: 'POST',
    headers,
...
export async function dismissThreatIntel(id: string, note?: string): Promise<{ ok: boolean; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/threats/dismiss', {
    method: 'POST',
    headers,
    body: JSON.stringify({ id, note }),
  });
...
export async function restoreThreatIntel(id: string): Promise<{ ok: boolean; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/threats/restore', {
    method: 'POST',
    headers,
...
export async function rollbackAiLearning(): Promise<{ ok: boolean; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/ai/rollback', { method: 'POST', headers, body: '{}' });
  if (!res.ok) return { ok: false, error: await parseApiError(res) };
  return { ok: true };
}
...
export async function fetchSemanticOutcomes(): Promise<SemanticOutcomesResponse> {
  const res = await mastyffAiFetch('/api/learning/semantic/outcomes');
  if (!res.ok) {
    return {
      records: [],
...
export async function labelSemanticOutcome(payload: {
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/learning/label', {
    method: 'POST',
    headers,
...
export async function rejectFp(payload: {
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/policy/fp/reject', {
    method: 'POST',
    headers,
    body: JSON.stringify(payload),
  });
...
export async function fetchAdminAuditTrail(): Promise<unknown[]> {
  const res = await mastyffAiFetch('/api/admin/audit-trail');
  if (!res.ok) return [];
...
export async function fetchLogs(): Promise<string[]> {
  const res = await mastyffAiFetch('/api/logs');
  if (!res.ok) return [];
...
export async function fetchSwarmLatest(): Promise<SwarmLatest | null> {
  const res = await mastyffAiFetch('/api/security-swarm/latest');
  if (!res.ok) return null;
  return (await res.json()) as SwarmLatest;
}

export async function fetchSwarmFigures(): Promise<SwarmFigureEntry[]> {
  const res = await mastyffAiFetch('/api/security-swarm/figures');
  if (!res.ok) return [];
...
export async function fetchVisualsLive(windowDays = 7, region?: string): Promise<VisualsLiveFetchResult> {
  const res = await mastyffAiFetch(`/api/visuals/live?${withWindowRegionQuery(windowDays, region)}`);
  if (!res.ok) {
    let message =
      res.status === 404
...
export async function fetchSwarmSummary(): Promise<string | null> {
  const res = await mastyffAiFetch('/api/security-swarm/summary');
  if (!res.ok) return null;
  return res.text();
}
...
export async function fetchThreatDiscoveryStatus(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/threat-discovery/status');
  if (res.status === 404) {
    return {
      status: null,
      error:
        'Threat Discovery API not found — run `pnpm exec tsc && pnpm dashboard:build`, then restart `pnpm dashboard:proxy mastyff-ai-configs/filesystem.json`.',
    };
  }
  if (res.status === 402) {
...
export async function fetchThreatAutomationSummary(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/threat-discovery/automation/summary');
  if (res.status === 404) {
    return {
      status: null,
      error:
        'Threat Discovery automation summary API not found — run `pnpm exec tsc && pnpm dashboard:build`, then restart `pnpm dashboard:proxy mastyff-ai-configs/filesystem.json`.',
    };
  }
  if (res.status === 402) {
    return { status: null, error: 'Pro license required (swarm feature).' };
...
export async function runThreatLab(
  ...
  const res = await mastyffAiFetch('/api/threat-discovery/threat-lab/run', {
    method: 'POST',
...
export async function runAutoThreatResearch(): Promise<{ ok: boolean; error?: string; jobId?: string }> {
  const res = await mastyffAiFetch('/api/threat-discovery/auto-research/run', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: '{}',
  });
...
export async function fetchThreatLabCandidate(id: string): Promise<ThreatLabCandidate | null> {
  const res = await mastyffAiFetch(`/api/threat-discovery/candidates/${encodeURIComponent(id)}`);
  if (!res.ok) return null;
  return (await res.json()) as ThreatLabCandidate;
}

export async function fetchThreatLabCandidates(): Promise<ThreatLabCandidate[]> {
  const res = await mastyffAiFetch('/api/security-swarm/threat-lab-candidates');
  if (!res.ok) return [];
...
export async function acceptThreatLabCandidate(id: string): Promise<boolean> {
  const res = await mastyffAiFetch('/api/security-swarm/threat-lab-candidates/accept', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id }),
  });
...
export async function rejectThreatLabCandidate(id: string): Promise<boolean> {
  const res = await mastyffAiFetch('/api/security-swarm/threat-lab-candidates/reject', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id }),
  });
...
export async function fetchAutoCorpusManifest(): Promise<AutoCorpusEntry[]> {
  const res = await mastyffAiFetch('/api/security-swarm/auto-corpus');
  if (!res.ok) return [];
...
export async function fetchSwarmLiveSession(): Promise<LiveFilesystemSession | null> {
  const res = await mastyffAiFetch('/api/security-swarm/live-session');
  if (!res.ok) return null;
...
export async function fetchAnalyticsSummary(
  ...
  const res = await mastyffAiFetch(`/api/analytics/summary?window=${encodeURIComponent(window)}`);
  if (!res.ok) return null;
...
export async function fetchSecurityDashboard(
  ...
  const res = await mastyffAiFetch(`/api/security/dashboard?window=${encodeURIComponent(window)}`);
  if (!res.ok) return null;
...
export async function quarantineAllThreats(): Promise<{
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/security/threats/quarantine', {
    method: 'POST',
...
export async function fetchSecurityQuarantinedThreats(
  ...
  const res = await mastyffAiFetch(`/api/security/threats/quarantined?days=${days}`);
  if (!res.ok) return [];
...
export async function restoreSecurityThreat(
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/security/threats/restore', {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
...
export type SetupMastyffAiConfig = {
  upstreamUrl?: string;
  listenPort?: number;
...
export type SetupStatusResponse = {
  ...
  totalSteps?: number;
  mastyffAiConfig?: SetupMastyffAiConfig & { done?: boolean };
...
export async function fetchSetupStatus(): Promise<SetupStatusResponse | null> {
  const res = await mastyffAiFetch('/api/setup/status');
  if (!res.ok) return null;
  return (await res.json()) as SetupStatusResponse;
}

export async function saveSetupMastyffAiConfig(body: {
  upstreamUrl: string;
  listenPort: number;
  authToken?: string;
}): Promise<{ ok: boolean; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/setup/mastyff-ai-config', {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
...
export async function fetchSetupCloudStatus(): Promise<SetupCloudStatus | null> {
  const res = await mastyffAiFetch('/api/cloud/status');
  if (!res.ok) return null;
...
export async function connectSetupCloud(body: {
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/cloud/connect', {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
...
export async function fetchAutopilotStatus(): Promise<AutopilotStatus | null> {
  const res = await mastyffAiFetch('/api/autopilot/status');
  if (!res.ok) return null;
...
export async function fetchSimilarEnvironmentBenchmarks(): Promise<SimilarEnvironmentBenchmarksResponse | null> {
  const res = await mastyffAiFetch('/api/benchmarks/similar-environment');
  if (!res.ok) return null;
...
export async function fetchContinuousAssuranceReport(): Promise<ContinuousAssuranceReport | null> {
  const res = await mastyffAiFetch('/api/assurance/continuous');
  if (!res.ok) return null;
...
export async function fetchLatestDigest(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/reports/digests/latest');
  if (!res.ok) return {};
  const body = (await res.json()) as {
    healthMarkdown?: string;
    securityJson?: Record<string, unknown>;
...
export async function generateDigestNow(): Promise<{ ok: boolean; error?: string }> {
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch('/api/reports/generate', { method: 'POST', headers });
  if (!res.ok) {
    const body = (await res.json().catch(() => ({}))) as { error?: string };
    return { ok: false, error: body.error || `HTTP ${res.status}` };
  }
...
export async function fetchPendingSuggestions(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/ai/suggestions/pending');
...
export async function fetchServerRegistry(): Promise<ServerRegistryEntry[]> {
  const res = await mastyffAiFetch('/api/servers/registry');
  if (!res.ok) return [];
...
export async function fetchAgenticDashboard(window = '7d'): Promise<AgenticDashboardResponse | null> {
  const res = await mastyffAiFetch(`/api/agentic/dashboard?window=${encodeURIComponent(window)}`);
  if (!res.ok) return null;
...
export async function fetchAgenticAudit(limit = 50): Promise<{
  ...
  const res = await mastyffAiFetch(`/api/agentic/audit?limit=${limit}`);
  if (!res.ok) return null;
  const body = (await res.json()) as {
    available?: boolean;
    records?: AgenticAuditRecord[];
...
export async function fetchAgenticDecisions(limit = 50): Promise<AgenticDashboardResponse['recentDecisions']> {
  const res = await mastyffAiFetch(`/api/agentic/decisions?limit=${limit}`);
  if (!res.ok) return [];
...
export async function fetchAgenticTasksDetail(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/agentic/tasks/detail');
  if (!res.ok) return null;
  const body = (await res.json()) as {
    stats?: { queued: number; running: number; completed: number; failed: number; total: number };
    pendingApprovals?: Array<{ requestId: string; toolName: string; description: string; createdAt: string }>;
...
export async function agenticPost(
  ...
  const headers = await buildMutatingHeaders();
  const res = await mastyffAiFetch(path, {
    method: 'POST',
    headers,
...
export async function fetchCertificationRegistry(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/certification/registry');
  if (!res.ok) return null;
  const body = (await res.json()) as {
    certifications?: Array<{ serverName: string; level: string; score: number; expiresAt: string }>;
    count?: number;
...
export async function fetchIndustryChainGraph(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/industry-standard/chain-graph');
  if (!res.ok) return null;
...
export async function fetchIndustryCapabilityGraph(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/industry-standard/capability-graph');
  if (!res.ok) return null;
...
export async function fetchIndustrySandboxTiers(): Promise<{
  ...
  const res = await mastyffAiFetch('/api/industry-standard/sandbox-tiers');
  if (!res.ok) return null;
...
export async function approvePlaybookAction(approvalId: string, approve: boolean): Promise<{ ok: boolean; error?: string }> {
  const res = await mastyffAiFetch('/api/agentic/playbook/approve', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchPlanComplianceAudit(): Promise<PlanComplianceReport | null> {
  const res = await mastyffAiFetch('/api/agentic/plan-compliance/audit');
  if (!res.ok) return null;
...
export async function fetchFederatedStatus(): Promise<FederatedStatus | null> {
  const res = await mastyffAiFetch('/api/agentic/federated/status');
  if (!res.ok) return null;
...
export async function aggregateFederatedDeltas(minContributors = 1): Promise<{ aggregated?: boolean; contributorCount?: number } | null> {
  const res = await mastyffAiFetch('/api/agentic/federated/aggregate', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function promoteFederatedRollout(): Promise<{ stage?: string; decision?: unknown } | null> {
  const res = await mastyffAiFetch('/api/agentic/federated/promote-rollout', { method: 'POST' });
  if (!res.ok) return null;
...
export async function fetchFederatedExportBundle(): Promise<unknown | null> {
  const res = await mastyffAiFetch('/api/agentic/federated/export');
  if (!res.ok) return null;
  return res.json();
}
...
export async function queryServerReputation(serverName: string, networkFetch = true): Promise<ReputationEntry | null> {
  const res = await mastyffAiFetch('/api/agentic/reputation/query', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function syncReputationMesh(): Promise<number> {
  const res = await mastyffAiFetch('/api/agentic/reputation/sync-mesh', { method: 'POST' });
  if (!res.ok) return 0;
...
export async function fetchZeroTrustScore(params: {
  ...
  const res = await mastyffAiFetch('/api/agentic/zero-trust/score', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchFleetChainGraph(sessionId?: string): Promise<ChainGraph | null> {
  ...
  const res = await mastyffAiFetch(`/api/agentic/fleet-chains/graph${q}`);
  if (!res.ok) return null;

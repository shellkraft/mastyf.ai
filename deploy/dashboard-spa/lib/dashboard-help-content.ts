...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  },
  {
    ...
    trigger: ['Threats → Threat Lab → Run Threat Lab', 'Env: SWARM_THREAT_LAB=true, MASTYF_AI_LLM_ENABLED'],
  ...
  },
  {
    ...
    category: 'Threats',
    what: 'Automated corpus fixture writes when confidence ≥ MASTYF_AI_THREAT_RESEARCH_MIN_CONFIDENCE (default 0.85). Policy is never auto-applied.',
    how: [
    ...
    trigger: ['Threats → Auto Research → Run', 'Env: MASTYF_AI_THREAT_RESEARCH_AUTO=true'],
  ...
  },
  {
    ...
    outputs: ['mastyf-ai-mcp-health-YYYY-MM-DD.md'],
  ...
  },
  {
    ...
    dataSources: ['~/.mastyf-ai/.threat-state.json', '~/.mastyf-ai/threat-intel-actions.jsonl'],
  ...
  },
  {
    ...
    dataSources: [
      ...
      'uncertainty-ranked queue (confidence near MASTYF_AI_SEMANTIC_MIN_CONFIDENCE)',
    ],
    outputs: [
      'debate verdicts',
      'eligibleTotal and remainingEligible counts',
      'optional auto-label when MASTYF_AI_TRIBUNAL_AUTO_LABEL=true',
    ],
  ...
  },
  {
    ...
    category: 'Settings',
    what: 'Checklist for Mastyf AI config, database health, proxy traffic, and optional cloud control plane link.',
    how: [
    ...
    dataSources: ['~/.mastyf-ai/setup.json', 'onboarding status', 'DATABASE_URL / history.db'],
    outputs: ['Saved proxy settings', 'cloud launch URL'],
    apis: [
      'GET /api/setup/status',
      'POST /api/setup/mastyf-ai-config',

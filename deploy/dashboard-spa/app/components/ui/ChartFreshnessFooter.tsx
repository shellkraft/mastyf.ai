...
export function ChartFreshnessFooter({ meta, generatedAt, recordCount, window: windowLabel, sparse }: Props) {
  ...
  if (win) parts.push(formatWindowSubtitle(win));
  if (isSparse) parts.push('sparse traffic');

  return (
    <p className="text-xs text-muted" style={{ marginTop: 'var(--space-3)' }} aria-live="polite">
      {parts.join(' · ')}
    </p>
  );
}

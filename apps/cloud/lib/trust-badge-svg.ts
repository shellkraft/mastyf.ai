...
export function resolveCloudBaseUrl(request?: Request): string {
  return resolveSiteUrl(request);
}

export function packagePathFromSegments(segments: string[]): string {
  const parts = [...segments];
  if (parts.length && parts[parts.length - 1]?.endsWith('.svg')) {
    parts[parts.length - 1] = parts[parts.length - 1]!.replace(/\.svg$/i, '');
  }

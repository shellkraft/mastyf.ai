...
describe('policy-subscriber', () => {
  ...
  beforeEach(() => {
    ...
    process.env = { ...env };
    process.env.MASTYF_AI_CONTROL_PLANE_URL = 'https://cloud.example.com';
    process.env.MASTYF_AI_CLOUD_API_KEY = 'gcp_test_key';
    process.env.MASTYF_AI_POLICY_TEMPLATES_DIR = join(dir, 'policy-templates');
    vi.resetModules();
  });
  ...
  it('disabled without control plane env', async () => {
    ...
    delete process.env.MASTYF_AI_CLOUD_API_KEY;

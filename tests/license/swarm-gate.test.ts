...
describe('Security Swarm Pro gate (v3)', () => {
  ...
  beforeAll(() => {
    const build = spawnSync('pnpm', ['build:guardian'], {
      cwd: REPO,
      stdio: 'pipe',
      encoding: 'utf8',
    });
    if (build.status !== 0) {
      throw new Error(`pnpm build:guardian failed:\n${build.stderr || build.stdout}`);
    }
  ...
  it('check-pro exits 1 without license key', () => {
    ...
    expect(r.stderr || r.stdout).toMatch(/MCP Guardian Pro required/i);
  });
  ...
  it('run.mjs exits before swarm work without license', () => {
    ...
    expect(r.stderr || r.stdout).toMatch(/MCP Guardian Pro required|license/i);
  });
});

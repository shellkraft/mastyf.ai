...
describe('McpProxyServer max inflight', () => {
  const prev = process.env['MASTYF_AI_PROXY_MAX_INFLIGHT'];

  afterEach(() => {
    if (prev === undefined) delete process.env['MASTYF_AI_PROXY_MAX_INFLIGHT'];
    else process.env['MASTYF_AI_PROXY_MAX_INFLIGHT'] = prev;
    vi.restoreAllMocks();
  });

  it('rejects tools/call when in-flight limit reached', async () => {
    process.env['MASTYF_AI_PROXY_MAX_INFLIGHT'] = '1';

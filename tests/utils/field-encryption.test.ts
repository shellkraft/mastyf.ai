...
describe('field-encryption', () => {
  const prev = process.env.MASTYFF_AI_DB_ENCRYPTION_KEY;

  beforeEach(() => {
    process.env.MASTYFF_AI_DB_ENCRYPTION_KEY = 'test-key-for-unit-tests-only';
  });

  afterEach(() => {
    if (prev === undefined) delete process.env.MASTYFF_AI_DB_ENCRYPTION_KEY;
    else process.env.MASTYFF_AI_DB_ENCRYPTION_KEY = prev;
  });
  ...
  it('passes through plaintext when key unset', () => {
    delete process.env.MASTYFF_AI_DB_ENCRYPTION_KEY;

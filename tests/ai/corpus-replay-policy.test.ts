...
describe('corpus replay policy split', () => {
  const prevPolicy = process.env.MASTYF_AI_POLICY_PATH;
  const prevReplay = process.env.MASTYF_AI_CORPUS_REPLAY_POLICY_PATH;

  beforeEach(() => {
    process.env.MASTYF_AI_POLICY_PATH = join(process.cwd(), 'policy-demo.yaml');
    process.env.MASTYF_AI_CORPUS_REPLAY_POLICY_PATH = join(process.cwd(), 'default-policy.yaml');
  });

  afterEach(() => {
    if (prevPolicy === undefined) delete process.env.MASTYF_AI_POLICY_PATH;
    else process.env.MASTYF_AI_POLICY_PATH = prevPolicy;
    if (prevReplay === undefined) delete process.env.MASTYF_AI_CORPUS_REPLAY_POLICY_PATH;
    else process.env.MASTYF_AI_CORPUS_REPLAY_POLICY_PATH = prevReplay;
  });

  it('defaults corpus replay path to default-policy.yaml', () => {
    delete process.env.MASTYF_AI_CORPUS_REPLAY_POLICY_PATH;

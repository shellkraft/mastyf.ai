...
- **Rate limits** (`maxCallsPerMinute`) — global, per expensive tool, and per-server examples. With `REDIS_URL` / Sentinel / Cluster, limits are enforced across all Mastyf AI replicas.
...
```bash
export MASTYF_AI_DAILY_BUDGET_USD=50
```

`CostAuditor.getDailySpendUsd()` and `isDailyBudgetExceeded()` read `call_records` since UTC midnight. Legacy alias: `MASTYF_AI_COST_BUDGET`.
